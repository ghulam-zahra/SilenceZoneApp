...
export default function App() {
  ...
  async function checkZone() {
    try {
      ...
      const apiZones = await fetchNearbyQuietZones(
        coords.latitude, coords.longitude, 10000
      );
      ...
      for (const zone of allZones) {
        ...
        if (distance <= zone.radius) {
          ...
          Alert.alert(
            `${zone.icon} Quiet Zone!`,
            `You entered ${zone.name}. Please silence your phone 🔕`,
            [{ text: 'OK' }]
          );
          ...
          await Notifications.scheduleNotificationAsync({
            content: {
              ...
              body: `You are near ${zone.name}. Please silence your phone 🔕`,
            },
            trigger: null,
          });
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.card,
          currentZone
            ? { backgroundColor: theme.alertCard, borderLeftColor: theme.red }
            : { backgroundColor: theme.safeCard, borderLeftColor: theme.green }
        ]}>
          {loading ? (
            ...
            <>
              <Ionicons
                name={currentZone ? getZoneIcon(currentZone.type) : 'location'}
                size={55}
                color={currentZone ? theme.red : theme.green}
                style={{ marginBottom: 10 }}
              />
              <Text style={[styles.zoneStatus, { color: theme.text }]}>
                {currentZone ? currentZone.name : "No Quiet Zone Nearby"}
              </Text>
              <Text style={[styles.zoneMessage, { color: theme.subtext }]}>
                {status}
              </Text>
            </>
          )}
        ...
        <View style={[styles.infoBox, { backgroundColor: theme.card }]}>
          <Text style={[styles.infoTitle, { color: theme.text }]}>
            Nearby Quiet Zones ({nearbyZones.length} found)
          </Text>
          {nearbyZones.length === 0 && !loading ? (
            <Text style={[styles.noZones, { color: theme.subtext }]}>
              No zones found in 10km range
            </Text>
          ) : (
        ...
        <View style={styles.footerRow}>
          ...
          <Text style={[styles.footer, { color: theme.subtext }]}>
            {loading ? " Fetching nearby zones..." : " Live — updates every 15 sec"}
          </Text>
        </View>
