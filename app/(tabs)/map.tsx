  ...
  const getMapHTML = () => {
    ...
    const markers = nearbyZones.map(zone => `
      L.circle([${zone.latitude}, ${zone.longitude}], {
        color: '${activeZone?.id === zone.id ? '#F44336' : '#2196F3'}',
        fillColor: '${activeZone?.id === zone.id ? '#F44336' : '#2196F3'}',
        fillOpacity: 0.2,
        radius: ${zone.radius}
      ...
      <body>
        ...
        <script>
          ...
          L.marker([${userLocation.latitude}, ${userLocation.longitude}], {
            icon: L.divIcon({
              html: '<div style="background:#10B981;width:16px;height:16px;border-radius:50%;border:3px solid white;box-shadow:0 0 6px rgba(0,0,0,0.5)"></div>',
              iconSize: [16, 16]
            })
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { backgroundColor: theme.background }]}>
        ...
        <Text style={[styles.headerSubtitle, { color: theme.subtext }]}>
          {nearbyZones.length} quiet zones nearby
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  badge: {
    marginHorizontal: 20, padding: 10, borderRadius: 12,
  ...
  map: { flex: 1 },
  legend: {
    flexDirection: 'row', justifyContent: 'center', padding: 12, gap: 20,
  },
